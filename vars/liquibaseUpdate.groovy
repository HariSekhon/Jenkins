  ...
  container('liquibase') {
    timeout (time: timeoutMinutes, unit: 'MINUTES') {
      ansiColor('xterm') {
        ...
        sh label: 'Liquibase Update',
          script: 'liquibase update --url="$JDBC_URL" --changeLogFile="$LIQUIBASE_CHANGELOG_FILE" --username="$LIQUIBASE_CREDENTIALS_USR" --password="$LIQUIBASE_CREDENTIALS_PSW"'
      }
    }
  }
}
